    ...
    def update_configs(self):
        try:
            ...
            if response.status_code != 200:
                ...
                acct, created = Account.objects.get_or_create(defaults={"chain_id":1},
                ...
                self.owner, created = Account.objects.get_or_create(defaults={"chain_id":1},
                    id=config.get("owner")
                )
                self.admins.clear()
                ...
                for deployer_id in config.get("whitelisted_deployers"):
                    self.whitelisted_deployers.add(
                        Account.objects.get_or_create(defaults={"chain_id":1},id=deployer_id)[0]
                    )
                self.save()
